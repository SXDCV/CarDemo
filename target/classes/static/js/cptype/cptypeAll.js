...
function getChangeName() {
	$("body").on('click', '#tab .cname', function() {
		var flag;
...
function getAdd() {
	$("#add").click(function() {
		layer.open({
			...
			maxmin:true,
			content:"cptype.do?method=toadd",
			end:function(){
				$("#tab").bootstrapTable('refresh');
			}
		});
...
function getAll() {
	$('#tab').bootstrapTable(
					{
						url :'cptype.do?method=getByPages', // 请求后台的URL（*）
						method : 'get', // 请求方式（*）
						dataType : 'json',
...
function edit(cid){
	layer.open({
		...
		skin:'layui-layer-lan',
		maxmin:true,
		content:"cptype.do?method=cptypeOne&cid="+cid,
		end:function(){
			$("#tab").bootstrapTable('refresh');
		}
	});
