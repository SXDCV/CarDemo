...
function getBlur() {
	...
	$("#username").blur(function() {
		var t=$(this).val();
		if (t.length==0) {
			 layer.tips('对不起，员工真实姓名不能为空！','#username',{tips:[2,'red']});
		}else if (!zu.test(t)) {
...
function getBtn() {
	$("#btn").click(function() {
		...
		if (uname.length==0) {
			...
			var xx="uname="+uname+"&upwd="+upwd+"&username="+username+"&utel="+utel+"&usex="+usex;
			$.post("User.do?method=UserAdd",xx,function(mydata){
				if (mydata==true) {
					//增加成功！
