...
function getChangeName() {
	$("body").on('click', '#tab .sname', function() {
		var flag;
		var zu=/^[\u0391-\uFFE5]+$/;
					...
					$.ajax({
						...
						success:function(mydata){
							if (mydata==1) {
								...
								// 提交到后台去
								$.ajax({
									url :'servicetype.do?method=updName',
									dataType : 'json',
									data : {
										sname : value,
										sid : id
...
function getAdd() {
	$("#add").click(function() {
		layer.open({
			...
			maxmin:true,
			content:"servicetype.do?method=toadd",
			end:function(){
				$("#tab").bootstrapTable('refresh');
			}
		});
...
function getAll() {
	$('#tab').bootstrapTable(
					{
						url :'servicetype.do?method=getByPages', // 请求后台的URL（*）
						method : 'get', // 请求方式（*）
						dataType : 'json',
...
function edit(sid){
	layer.open({
		...
		skin:'layui-layer-lan',
		maxmin:true,
		content:"servicetype.do?method=servicetypeOne&sid="+sid,
		end:function(){
			$("#tab").bootstrapTable('refresh');
		}
	});
